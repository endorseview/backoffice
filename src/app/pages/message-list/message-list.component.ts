...
import { MessageList } from 'app/model/admin/message-list.model';
...
export class MessageListComponent implements OnInit {
  ...
  totalRecords: any;
  public list: Data[];
  public search: MessageList = new MessageList();
  public pageable: Pageable = new Pageable();
  public title: string;
  public message: string;
  public cols = [
    { field: 'id', header: 'table.columns-title.id', width: '100px' },
    { field: 'title', header: 'table.columns-title.title', width: '300px'},
    { field: 'message', header: 'table.columns-title.message'},
    { field: 'code', header: 'table.columns-title.code', width: '100px'},
  ...
  findAll(event: LazyLoadEvent): void {
    this.spinner.show();
    this.messageListService.findAll(event, this.pageable, this.search).subscribe(data => {
      this.spinner.hide();
  ...
  edit(item: any): void {
    this.router.navigate(['message-list', item.id]);
  }
  add(): void {
    this.router.navigate(['message-list', 'add']);
  }
  remove(item): void {
    this.selectedItems = [];
  ...
  delete(item): void {
    this.spinner.show();
    this.messageListService.delete(item.id)
      .subscribe(() => {
        this.spinner.hide();
